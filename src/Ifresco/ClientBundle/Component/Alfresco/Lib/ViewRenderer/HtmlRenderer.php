...
class HtmlRenderer implements ViewRenderer
{
    ...
    public function render($node, $userObj)
    {
        ...
        if ($contentData != null) {
            return $this->renderView($height, $contentData->getContent(), $node->getContent()->getMimetype());
        }
    ...
    private function renderView($height, $content, $mimeType = '')
    {
        $heightStyle = (!empty($height) ? 'max-height:' . $height.';' : 'max-height:300px');

        if('text/html' == $mimeType) {
            $html = '<div id="HtmlRenderer" style="width:100%;' . $heightStyle . '">' . $content . '</div>';
        } else {
            $html = '<iframe id="HtmlRenderer" style="width:100%;' . $heightStyle . '">' . $content . '</iframe>';
        }

        return $GLOBALS['kernel']->getContainer()->get('twig')->render('IfrescoClientBundle:Viewer:HtmlRenderer.html.twig', array(
        	'content' => $content,
        	'mimeType' => $mimeType
        ));
    }
