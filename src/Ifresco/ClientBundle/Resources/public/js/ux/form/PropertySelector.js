 ...
 */
Ext.define('Ext.ux.form.PropertySelector', {
    ...
    alternateClassName: 'Ext.ux.PropertySelector',
    alias: 'widget.propertyselector',
    ...
    valueField: 'id',
    dragGroup: 'DDPropertySelector',
    dropGroup: 'DDPropertySelector',
    ...
    setupItems: function() {
        ...
        me.boundList.on('itemclick', me.onItemClick, me);
        me.boundList.on('itemadd', me.onItemAddOrRemove, me);
        me.boundList.on('itemremove', me.onItemAddOrRemove, me);
    ...
    initData: function() {
    	Ext.each(this.boundList.all.elements, function(el) {
	    	Ext.each(el.getElementsByTagName("input"), function(input) {
	        	switch(input.getAttribute('type')) {
	        		case 'text':
	        			input.click(); // TO CREATE ELEMENT
	        		break;
	        	}
	        });
    	});
    	this.boundList.focus();
    },

    onItemAddOrRemove: function() {
        var index = 0;
        Ext.each(this.boundList.all.elements, function(el) {
            var className = '';

            var rxp = new RegExp("\\s?\\b(even|odd)\\b", "g");
            className = el.firstChild.className.replace(rxp, '');
    ...
    onItemClick: function(view, record, el, index, e) {
        var target = e.target,
        ...
        if (cls && cls.indexOf('x-tool-close') > -1) {
            ...
            record.fireEvent('afterremove', record);
            return false;
        }

        if (target.tagName == 'INPUT') {
            fieldName = target.getAttribute('data-field');
            switch(target.getAttribute('type')) {
                case 'checkbox':
                    field = record.data[fieldName];
                    value = target.value;
                    if (target.checked) {
                        if (Ext.isArray(field)) {
                            if (! Ext.Array.contains(field, value)) {
                                field.push(value);
                            }
                        } else {
                            record.data[fieldName] = true;
                        }

                    }
                    else {
                        if (Ext.isArray(field)) {
                            Ext.Array.remove(field, value);
                        } else {
                    ...
                    return false;
                case 'text':
                	console.log("TEXT FIELD IN PROP SELECT");
                    field = record.data[fieldName];
                    value = target.value;
