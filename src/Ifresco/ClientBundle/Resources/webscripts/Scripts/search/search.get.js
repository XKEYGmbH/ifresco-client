<import resource="/Company Home/Data Dictionary/Web Scripts/at/xkey/ifresco/search/evaluator.lib.js">
<import resource="/Company Home/Data Dictionary/Web Scripts/at/xkey/ifresco/doclib/filters.lib.js">
<import resource="/Company Home/Data Dictionary/Web Scripts/at/xkey/ifresco/search/parse-args.lib.js">
 ...
 */
function getRepositoryItem(folderPath, node,params)
{
		 ...
		 if (item !== null)
		  {
            ...
            item.location = Common.getLocation(locationNode, companyhome); // companyhome
            // Parent node
 ...
 */
function getItem(siteId, containerId, pathParts, node, params)
{
   var item = null;
   if (siteId == null)
   {
      item = getRepositoryItem(pathParts, node, params);
   }
   else
   {
      switch ("" + containerId)
 ...
 */
function processResults(nodes, columns, maxResults, total, params)
{
   // empty cache state
   processedCache = {};
   var results = [],
   ...
   for (i = 0, j = nodes.length; i < j && added < maxResults; i++)
   {
      ...
      item = getItem(parts[0], parts[1], parts[2], nodes[i], params);
      if (item !== null)
      {
         results.push(item);
         added++;
...
function getSearchResults(params)
{
   ...
   if (formData !== null && formData.length !== 0)
   {
      ...
      if (formQuery.length !== 0 || ftsQuery.length !== 0)
      {
         ...
         if (formJson.datatype !== null && typeof formJson.datatype !== 'undefined') 
   ...
   if (ftsQuery.length !== 0)
   {
      ...
      var queryDef = {
         ...
         page: {maxItems: params.maxResults, skipCount: params.skip},    // allow for space for filtering out results
	  ...
	  var start = 0;
	  var total = parseInt(nodes.length)+parseInt(params.skip);
	  if (params.max != null)
		nodes = nodes.splice(start, params.max);
   }
   else
   {
	  var total = 0;
   ...
   logger.warn(ftsQuery);
   return processResults(nodes, params.columns, params.maxResults, total, params);
}
...
function main()
{
   var params =
   {
      ...
      columns: (args.columns !== null) ? args.columns : null,
	  skip: (args.skip !== null) ? args.skip : 0,
	  max: (args.max !== null) ? args.max : null,
